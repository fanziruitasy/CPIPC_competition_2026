...
def xlsx_sheets(path: Path) -> list[dict]:
    try:
        ...
        if not _has_invalid_auto_filter_cause(exc):
            raise
        repaired = _remove_invalid_auto_filters(path)
        values_wb = openpyxl.load_workbook(
            io.BytesIO(repaired), data_only=True, read_only=False, keep_links=False
        )
        formulas_wb = openpyxl.load_workbook(
            io.BytesIO(repaired), data_only=False, read_only=False, keep_links=False
        )
    sheets = []
    try:
...
def _has_invalid_auto_filter_cause(error: ValueError) -> bool:
    current: BaseException | None = error
    while current is not None:
        if "Value does not match pattern" in str(current):
            return True
        current = current.__cause__
    return False


def _remove_invalid_auto_filters(path: Path) -> bytes:
    """在内存副本中删除无效筛选范围，不改动转换产物和单元格。"""
    output = io.BytesIO()
    paired = re.compile(rb'<autoFilter\b[^>]*\bref="(?P<ref>[^"]*)"[^>]*>.*?</autoFilter>', re.DOTALL)
    empty = re.compile(rb'<autoFilter\b[^>]*\bref="(?P<ref>[^"]*)"[^>]*/>')

    def keep_valid(match: re.Match[bytes]) -> bytes:
        reference = match.group("ref").decode("utf-8", errors="replace")
        try:
            openpyxl.worksheet.filters.AutoFilter(ref=reference)
        except ValueError:
            return b""
        return match.group(0)

    with zipfile.ZipFile(path) as source_zip, zipfile.ZipFile(
        output,
        "w",
        ...
        for item in source_zip.infolist():
            ...
            if item.filename.startswith("xl/worksheets/") and item.filename.endswith(".xml"):
                payload = paired.sub(keep_valid, payload)
                payload = empty.sub(keep_valid, payload)
...
def xls_sheets(path: Path) -> list[dict]:
    ...
    for provider in providers:
        try:
            ...
            break
        except Exception as exc:  # 不同机器安装的 OLEDB Provider 可能不同。
    ...
    try:
        for name in tables:
            ...
            try:
                ...
                values = [list(row) for row in zip(*raw, strict=False)] if raw else []
            finally:
                if recordset.State:
                    recordset.Close()
    ...
    if truncated:
        # ACE 可能截断超过255字符的 BIFF 文本，必要时用 Excel COM 补读全文。
        try:
