...
def load_source(path: Path) -> tuple[list[dict], str, bytes]:
    ...
    # ACE may reject an .xls currently open in Excel. A private copy removes that lock.
    with TemporaryDirectory() as directory:
        copy = Path(directory) / "source.xls"
        shutil.copy2(path, copy)
        # ponytail: ADO exposes cached .xls values only; use a BIFF parser if raw formulas become audit-critical.
